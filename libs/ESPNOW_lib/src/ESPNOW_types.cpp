...
int ESPNOW_packet::toBytes(uint8_t *bytes, int max_len) {
  int correct_len =
      sizeof(ESPNOW_packet) + this->wlan.actionframe.content.length - 0xff;

  assert(correct_len <= max_len);

  memcpy(bytes, this, correct_len);

  memcpy(bytes + correct_len - sizeof(this->wlan.fcs), &(this->wlan.fcs),
         sizeof(this->wlan.fcs));
...
int ESPNOW_packet::get_payload_len(uint8_t *raw_bytes, int len) {
  ...
  if (len < radiotap_len + WLAN_LEN + ACTIONFRAME_HEADER_LEN + 1)
    return -1;
...
uint8_t *ESPNOW_packet::get_payload(uint8_t *raw_bytes, int len) {
  ...
  if (len < radiotap_len + WLAN_LEN + ACTIONFRAME_HEADER_LEN +
                VENDORSPECIFIC_CONTENT_LEN)
    return NULL;

  return raw_bytes + radiotap_len + WLAN_LEN + ACTIONFRAME_HEADER_LEN +
         VENDORSPECIFIC_CONTENT_LEN;
}
