...
void ESPNOW_manager::set_filter(uint8_t *src_mac, uint8_t *dst_mac) {
  // sudo tcpdump -i wlp5s0 'type 0 subtype 0xd0 and wlan[24:4]=0x7f18fe34 and
  ...
  uint8_t jeq_dst =
      dst_mac == NULL ? 0x30 : 0x15; // 0x30 jump if >=. 0x15 jump if ==.
  ...
  struct sock_filter temp_code[this->bpf.len] = {
      ...
      {0x40, 0, 0, 0x00000012},   {jeq_dst, 0, 26, LSB_dst},
      {0x48, 0, 0, 0x00000010},   {jeq_dst, 4, 24, MSB_dst},
      {0x40, 0, 0, 0x00000006},   {jeq_dst, 0, 22, LSB_dst},
      {0x48, 0, 0, 0x00000004},   {jeq_dst, 0, 20, MSB_dst},
      {0x50, 0, 0, 0x00000001},   {0x45, 0, 13, 0x00000002},
      {0x45, 0, 4, 0x00000001},   {0x40, 0, 0, 0x0000001a},
      {jeq_src, 0, 15, LSB_src},  {0x48, 0, 0, 0x00000018},
      {jeq_src, 12, 13, MSB_src}, {0x40, 0, 0, 0x00000012},
      {jeq_src, 0, 11, LSB_src},  {0x48, 0, 0, 0x00000010},
      {jeq_src, 8, 9, MSB_src},   {0x40, 0, 0, 0x00000006},
      {jeq_dst, 0, 7, LSB_dst},   {0x48, 0, 0, 0x00000004},
      {jeq_dst, 0, 5, MSB_dst},   {0x40, 0, 0, 0x0000000c},
...
void ESPNOW_manager::start() {
  ...
  if (bpf.len > 0) {
    filter_errno =
        setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, &(this->bpf), sizeof(bpf));
    assert(filter_errno >= 0);
  }

  priority_errno =
      setsockopt(fd, SOL_SOCKET, SO_PRIORITY, &(this->socket_priority),
                 sizeof(this->socket_priority));
  ...
  pthread_create(&recv_thd_id, NULL, &(ESPNOW_manager::sock_recv_thread),
                 &recv_thread_params);
}
...
void *ESPNOW_manager::sock_recv_thread(void *p_arg) {
  ...
  while (1) {
    raw_bytes_len = recvfrom(params.sock_fd, raw_bytes, LEN_RAWBYTES_MAX,
                             MSG_TRUNC, NULL, 0);
...
int ESPNOW_manager::send(uint8_t *payload, int len) {
  ...
  //	copy the payload in the packet array and then copy it back into the
  // buffer...
