...
public:
  ...
  ESPNOW_manager(char *interface, uint8_t datarate, uint16_t channel_freq,
  ...
  void set_recv_callback(void (*callback)(uint8_t src_mac[6], uint8_t *data,
                                          int len));
  ...
  void set_channel(uint16_t channel_freq) {
    mypacket.set_channel(channel_freq);
  }
